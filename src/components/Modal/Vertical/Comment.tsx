...
function CommentItem({setRefresh,token,currentUser,commentId,name,comment,userId,imagePath}:ICommentItem){
  const onClickDelete = (e:any) => {
      axios.delete(`${SERVER_API}/forms/comments/${commentId}`,
        {
          headers : {
...

        {
          currentUser === userId
            ...
            {/*<span className={'comment-item-modify'}>수정</span>*/}
              <span onClick={onClickDelete} className={'comment-item-delete'}>삭제</span>
            </div>
            :null
        }
      </div>
      ...
      <div className={'comment-item-list-outer-container'}>
        <div className={'comment-item-list-inner-container'}>
          <div className={'comment-item-register-comment-container'}>
              <div className={'comment-item-img-container'}>
                <img className={'comment-item-img'} src={user!==null && user.image!=='' ?user.image : '/img/profile_default.png'} alt={'user profile'} />
              </div>
            <div className={'comment-item-text-container'}>

              <div className={'comment-item-comment-container'}>
                <input id={"commentInput"} ref={inputRef} onKeyPress={addCommentEvent} onChange={(e)=>{setComment(e.currentTarget.value)}} className={'comment-modal-input'} placeholder={"댓글 추가..."} />
              </div>
            </div>
          </div>
          <div>
            {
              comList !== null ? comList.map((item)=>{
              // commentList.map((item)=>{
                return(
                  <CommentItem setRefresh={setRefresh} token={accessToken} commentId={item.id} currentUser={currentUser} name={item.user.nickname} comment={item.content} userId={item.user.id} imagePath={item.user.image} />
                )
              }) : null
            }
          </div>
